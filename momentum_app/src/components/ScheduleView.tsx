      ...
      <main className="max-w-[1400px] mx-auto px-[15px] md:px-[40px] lg:px-[60px] pb-[80px]">
        ...
        <div className="mt-[24px] md:mt-[32px] mb-[32px] md:mb-[40px]">
          <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 md:gap-6">
            <h2 className="text-[#002244] text-[32px] md:text-[40px] lg:text-[48px] font-['Open_Sans',_sans-serif] lowercase">
              Team Schedule
            </h2>
            ...
            <select 
              ...
              className="w-full md:w-[240px] lg:w-[280px] h-[40px] md:h-[44px] bg-white border border-[#d9d9d9] rounded-[8px] px-[16px] text-[16px] font-['Open Sans',_sans-serif] lowercase"
            >
        ...
        {sortedEvents.length > 0 ? (
          <section className="space-y-[20px] md:space-y-[24px]">
            {sortedEvents.map((event, index) => (
              <div key={index} className="flex flex-col sm:flex-row sm:items-start gap-[12px] sm:gap-[16px] md:gap-[24px] pb-[20px] md:pb-[24px] border-b border-[#2A323F]/20 last:border-b-0">
                {/* Time Column */}
                <section className="flex text-[#002244] text-[48px] md:text-[56px] lg:text-[64px] font-['Playfair_Display_SC',_serif] leading-none sm:min-w-[80px] md:min-w-[100px] lg:min-w-[120px] gap-1" style={{ alignItems: 'self-end' }}>
                  <span>{event.time}</span>
                  <span className="text-[24px] font-['Playfair_Display_SC',_serif] pt-1" style={{ fontWeight: 'bold' }}>{getTimePeriod(event.time)}</span>
                </section>
